...
初回起動では「思考の保存フォルダを選ぶ」オンボーディングだけを出します。「フォルダを選んでMarkdownを作成」や「Markdownを開いて編集」といった開発用ボタンは通常画面に出しません。

「保存フォルダを選ぶ」からSAFのフォルダ選択でDocumentsなどを選びます。空フォルダなら一覧の作成ボタンで`新規マインドマップ.md`が作られます。同名がある場合は`新規マインドマップ (1).md`のように空き番号を使います。作成後に編集画面が開き、読み込み・編集・自動保存ができることも確認します。保存に失敗した場合、元のファイルが空にならないことを確認します。PresentationはSAFを直接呼びません。
