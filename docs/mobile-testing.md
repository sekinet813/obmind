...
ホーム画面の「フォルダを選んでMarkdownを作成」から、SAFのフォルダ選択でDocumentsなどを選び、その場所へ`新規マインドマップ.md`が作られることを確認します。同名がある場合は`新規マインドマップ (1).md`のように空き番号を使います。作成後に編集画面が開き、読み込み・編集・保存ができることも確認します。保存に失敗した場合、元のファイルが空にならないことを確認します。PresentationはSAFを直接呼びません。
