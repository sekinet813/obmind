...
- [x] 新規作成時のファイル名を`新規マインドマップ`にし、同名があれば`(1)` `(2)`と連番を付ける
...
- [x] ソフトウェアキーボードで編集中Nodeが隠れないように、Viewportを動かす
...
- [x] ユーザー向け表記の「正本フォルダ」を「保存フォルダ」に統一する
...
- [x] 編集画面AppBarの設定ボタンを削除する。設定は一覧側から開けるようにする
...
- [x] 色パターンとマインドマップの形を組み合わせたテンプレートを数種用意し、UIから適用できるようにする
...
- [x] Library検索がファイル名だけでなく、地図内のNode textにもマッチするようにする
...
- [x] Libraryの表示を、ファイル名のみのリストと、プレビュー付きタイルの2通りから選べるようにする
...
- [x] Library一覧のタイトル左側にObmindのアプリアイコンを表示する
...
## P1: Phase 13 親ノード・文言・デザインテーマ

日常編集の分かりやすさを優先する。デザイン変更は色と形（Node / Edge / 背景）に限定し、水平・放射レイアウトは切り替えない。Play Store提出そのものはPhase 14。

### T-083 親ノードとファイル名を揃える

- [x] Root（親ノード）のテキストをファイル名と同期する。Rootを変更したらファイル名も揃える

背景:

- 一覧の表示名はファイル名、地図の親はH1（Root）で、現状は別々に変わる
- 新規作成のファイル名は`新規マインドマップ`だが、本文H1は`# Obmind`のまま（`pocMarkdownContents`）
- T-056は「Markdown内容は変えず、ファイル名のみ変更する」だった。地図の題名としては親ノードとファイル名が一致した方が分かりやすい

完了条件:

- アプリから新規作成した地図は、ファイル名の題名（拡張子を除く）とRoot textが一致する。既定は`新規マインドマップ`（連番時は`新規マインドマップ (1)`など、ファイル名と同じ）
- キャンバスでRoot textの編集を確定したとき、ファイル名も同じ題名へrenameする（入力1文字ごとに即renameしない）
- 一覧からの名前変更（T-056）でもRoot textを同じ題名へ更新する。Autosaveまたは同一保存経路でMarkdownへ書き、無条件上書きや空ファイル化をしない
- 不正文字・空・重複は既存の`MindMapFileName`規則に合わせる。重複時は既存ファイルを上書きせず、拒否するか連番で退避する（Data Loss防止）
- 開いた直後にファイル名とH1が違う既存ファイルを、黙ってrenameしたり本文を書き換えたりしない
- Recent一覧と開いている地図の参照がrename後も整合する
- PresentationがOS APIを直接呼ばない。`MindMapStorage`経由にする
- 座標をDomainへ保存しない
- ユニットテストがある（新規作成の一致、Root確定後のrename、不正名、既存不一致ファイルを開いただけでは変えない）

対象外:

- 外部（Obsidian等）でファイル名だけ変えた場合の自動追従
- Format v0.1のH1=Root契約の破壊
...
Roadmap: Phase 13 親ノードとファイル名の同期

### T-084 マインドマップ編集画面の保存ボタンを削除する

- [x] 編集画面AppBarの手動「保存」ボタンを外す。Autosaveに任せる

背景:

- T-034のAutosaveが既定であり、手動保存は重複している
- AppBarに「保存」があると、自動保存なのに押さないと危ないように見える

完了条件:

- `MindMapPage`から手動保存ボタン（`saveMindMap` / `saveMarkdown`）を削除する
- Autosave、外部変更検知、未対応Markdownの読み取り専用表示は維持する
- 「自動保存」「保存中」など状態表示は、過剰でなければ残してよい
- 生Markdown編集画面（`MarkdownEditorPage`）まで無理に消さない。通常のマインドマップ編集導線に出していなければ対象外
- 関連Widgetテストを更新する。保存失敗時にデータを空にしないテストは残す
...
Roadmap: Phase 13 編集画面の保存ボタン削除

### T-085 ユーザー向けメッセージから「正本」を外す

- [x] 画面・ダイアログ・SnackBarなどユーザーが見る文言から「正本」を削除する

背景:

- T-077で「正本フォルダ」は「保存フォルダ」に変わったが、`homeMessage`やオンボーディング本文などに「正本」が残っている
- プロダクト原則としてはMarkdown is Source of Truthでよいが、ユーザー向けには馴染みにくい

完了条件:

- `app_ja.arb`および生成l10n、関連Widgetテストの期待文字列から、ユーザー向けの「正本」をなくす
- 代替は「Markdownファイルとして残る / 保存する」など、保存先がユーザーのファイルであることが分かる表現にする
- 日本語と英字の間に半角スペースを入れない
- `AGENTS.md`、`docs/requirements.md`、ADRなど開発者向けdocsの原則文言までは書き換えない（T-077と同じ線引き）
...
Roadmap: Phase 13 正本表記の削除

### T-086 デザイン変更でLayoutを変えない

- [x] デザイン（テーマ）切替は色とNode / Edgeの見た目だけにし、水平・放射は変更しない

背景:

- T-079のデザインテンプレートは`theme`と`layout`をセットで適用する（例: ソフト水平、シンプル放射）
- レイアウトは地図の構造の見せ方であり、デザインテーマとは分けたい
- 水平 / 放射の切替自体は既存のレイアウトメニューで足りる

完了条件:

- デザインメニュー（または同等のUI）を選んでも`layout`を書き換えない。Horizontal / Radialは現状のまま
- レイアウト切替の既存UIは残す。デザイン画面へ統合して隠さない
- 既存ファイルの`layout`をデザイン適用だけで変えない
- 関連するl10n（「シンプル放射」など形を含む名称）を、テーマ名だけに直す
- WidgetテストまたはCanvas Themeテストを更新する
- Domainに色やWidgetを置かない。座標をDomainへ保存しない
...
Roadmap: Phase 13 デザインとLayoutの分離

### T-087 デザインテーマを4種に拡充する

- [x] キャンバスのデザインテーマを、現行・インク・ダーク・ミニマルの4種にする

背景:

- 現状の`MindMapThemeId`は`minimal` / `soft` / `dark`の3種
- ユーザーが選ぶのはデザインテーマであり、水平・放射のプリセットではない（T-086）
- Obsidianに寄せたダークノート風の見た目は欲しいが、UI・ストア・docsで「Obsidian」という名称は使わない（Obsidian専用アプリにしない原則）

完了条件:

- 次の4種がキャンバス上で視覚的に区別できる。識別子はFrontmatterの`theme`として保存する
  - ペーパー: 現行のBrand寄りの見た目（既存`soft`を維持してよい）
  - インク: ダークノート風（Obsidianに寄せる。識別子例: `inkwell`。表示名にObsidianを使わない）
  - ダーク: 既存`dark`
  - ミニマル: 既存`minimal`
- マインドマップ編集画面から4種を選べる。適用はAutosave経由。Markdown手編集を要求しない
- デザイン適用で`layout`を変えない（T-086）
- 新しい`theme`値を足す場合は`docs/markdown-format.md`と`docs/requirements.md`のTheme節を更新する。未知値は黙って捨てず、既存の未知キー保持方針に合わせる
- 未指定ファイルの見た目を勝手に変えない。既存の`minimal` / `soft` / `dark`の読み書きを壊さない
- Domainに色やWidgetを置かない。課金ロックはしない
- WidgetテストまたはCanvas Themeテストがある
...
Roadmap: Phase 13 デザインテーマ4種

## P1: Phase 14 Play Storeリリース準備

AndroidのGoogle Play提出に必要な準備。独自バックエンド、アカウント、課金は対象外。本番Application IDは[ADR-0003](decisions/ADR-0003-identifiers.md)どおり人間が決める（T-007）。秘密情報や本番IDが未確定ならBLOCKEDにし、独立したタスクを先に進める。

### T-088 設定にアプリ情報を置く

- [x] 設定画面でバージョンを確認でき、オープンソースライセンスを見られるようにする

背景:

- Play掲載や問い合わせ時に、端末上でバージョンを確認できる必要がある
- Flutter / 依存パッケージのライセンス表示はストア審査で求められることがある

完了条件:

- 設定画面にアプリ名、`versionName`（`pubspec`のversion）を表示する
- 依存パッケージのライセンス一覧へたどり着ける（`LicensePage`または同等）
- パスや内部IDをDomainモデルへ載せない
- Widgetテストがある
...
Roadmap: Phase 14 アプリ情報とライセンス

### T-089 本番向けの開発用導線を通常UIから外す

- [x] リリース利用者向けに、PoCや生Markdown編集の導線を通常画面から出さない

背景:

- HomeにVault未配線時の「フォルダを選んでMarkdownを作成」「Markdownを開いて編集」が残っている
- 新規地図のテンプレート定数名が`pocMarkdownContents`のまま
- 通常UIでMarkdown編集を要求しない、という原則と揃える

完了条件:

- 通常のLibrary / 編集 / 設定の導線から、生MarkdownエディタとPoC用ボタンを外す
- Vault未設定時はオンボーディング（T-061）だけを出す。開発用フォールバックが本番UIに残らない
- マインドマップの読み書き・Autosaveは維持する。Markdown互換は壊さない
- テスト用にWidgetへuse caseを注入する経路は残してよい
- 関連Widgetテストを更新する
...
Roadmap: Phase 14 本番UIからPoC導線を外す
...
- アカウントも独自サーバーも無いが、ユーザーが選んだフォルダのMarkdownを読み書きする
- Play ConsoleはData safetyの申告が必要。公開URLが空だと提出できない場合がある

完了条件:

- `docs/privacy-policy.md`（または同等）に、収集しないデータ、ファイルは端末とユーザー指定フォルダに留まること、第三者送信が無いことを日本語で書く
- Play Data safety向けに、ファイルアクセスの目的（マインドマップの保存）を短く整理する
- アプリ内設定からポリシーへ辿れるようにする。公開URLが未定なら、アプリ内表示だけでもよい
- 公開用ホスティングURLとPlay Consoleへの転記は人間作業。URL未定ならこのタスクの公開リンク部分は残課題とし、文面とアプリ内表示が揃えば完了してよい
...
Roadmap: Phase 14 プライバシーポリシー

### T-091 Play掲載用の文言とスクリーンショット手順

- [x] ストア掲載に使う短い説明・詳細・スクリーンショットの取り方をdocsへ置く

背景:

- アイコン（T-064）はあるが、Playの短い説明、詳細説明、フィーチャーグラフィック、端末スクリーンショットが未整備
- 実機撮影はエージェント環境ではできないことが多い

完了条件:

- `docs/play-store.md`（または同等）に次を書く
  - 短い説明（80文字以内の下書き）
  - 詳細説明の下書き（日本語。Markdownをユーザーのファイルとして残る点、アカウント不要を含む）
  - 必要な画像サイズ（アイコン、フィーチャーグラフィック、Phoneスクリーンショット）
  - 撮影すべき画面（オンボーディング、一覧、放射マップ、テーマ違い、編集）
- アプリ内の「正本」は使わない（T-085と整合）
- 実機スクリーンショットそのものの作成は、環境が無ければ手順だけ残しBLOCKEDにしない。画像未撮影は残課題として明記する
...
- `android/app/build.gradle.kts`のreleaseはdebugキーを使っている
- Playへの新規公開はAAB（`.aab`）が基本。keystoreはリポジトリに置かない

完了条件:

- releaseビルドが`key.properties`等のローカル設定を読む。ファイルはgitignoreする。サンプル（`key.properties.example`）を置く
- 秘密鍵・パスワードをコミットしない。エージェントはkeystoreを新規発行してリポジトリへ入れない
- `flutter build appbundle`の手順を`docs/play-store.md`または`docs/mobile-testing.md`に書く
- keystoreが環境に無い場合は、配線と手順までを完了とし、実ファイル作成はBLOCKED / 人間作業としてReasonを残す
- `docs/mobile-testing.md`の「Play Store配布はこのリポジトリの標準対象外」を、手順docsへの参照に更新する

依存:

- T-007（提出そのものは本番ID確定後。配線と手順はこのタスクで先に進めてよい）

Roadmap: Phase 14 Release署名とAAB
...
- Playはtarget APIとネイティブライブラリの16KBページサイズを要求する
- Flutter / NDKの既定に乗るが、提出前に明示確認が必要

完了条件:

- 使用中のFlutterが要求する`targetSdk` / `compileSdk`と、Playの現行要件を`docs/play-store.md`に記録する
- 不足があれば`compileSdk` / NDK / Flutterチャネルの上げ方を残す。勝手に大規模アップグレードしない。判断が必要ならBLOCKED
- 16KBページサイズは公式の確認手順（またはFlutterの案内）に沿って結果を書く。環境で測れない場合は手順と未確認である旨を残す
- DomainやMarkdown Formatは変更しない

依存:

- T-092

Roadmap: Phase 14 Play技術要件

### T-094 Play提出前チェックリスト

- [x] 人間がPlay Consoleで行う作業と、アプリ側の完了条件を1枚にまとめる

背景:

- T-007（本番ID）、コンテンツのレーティング、Data safety転記、スクリーンショットアップロード、内部テストトラックは人間作業
- エージェントが提出ボタンを押したり、本番IDを勝手に決めたりしない

完了条件:

- `docs/play-store.md`にチェックリストがある。少なくとも次を含む
  - T-007の本番applicationIdが設定済み
  - Release AABが本番ID・本番署名である
  - プライバシーポリシーURL
  - Data safety申告
  - レーティング、カテゴリ、連絡先メール
  - スクリーンショットとフィーチャーグラフィック
  - Android実機での基本操作確認（T-048）
- チェックリストを埋めること自体（Console操作）は人間作業。ドキュメントが揃えばこのタスクは完了とする
...
### T-097 ノード本文と展開ボタンの配置改善

- [x] ノード本文を中央揃えの2行表示にし、展開ボタンを接続線側へ移す

完了条件:

- ノード本文は中央揃えで最大2行まで折り返す
- 長い本文でもフォントサイズを縮小せず、省略表示する
- 既定ノードサイズを2行表示に必要な大きさへ広げる
- 折りたたみ / 展開ボタンは本文と重ならず、枝（接続線）の方向の付け根に丸く表示する
- ボタンの見た目の大きさはそのままに、タップしやすい反応範囲を確保する
- ボタンTapによる折りたたみ / 展開を維持する

依存:

- T-055

Roadmap: Phase 17 ノードの可読性
