...
アプリ全体のライト / ダークを設定から選べるようにする。詳細な完了条件は[tasks.md](tasks.md)のT-096を参照。

- [x] アプリ全体のテーマ設定（端末 / ライト / ダーク）
  - Task: T-096

## Phase 17：ノードの可読性

ノード本文を読みやすくし、折りたたみ / 展開操作が本文を邪魔しない配置にする。詳細な完了条件は[tasks.md](tasks.md)のT-097を参照。
