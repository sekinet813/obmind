...
このリポジトリは、Android / iOS向けFlutterアプリのテンプレートです。

コピー直後は最小の日本語ホーム画面だけがあります。アプリ固有の仕様は、コピー後にこの節を書き換えてください。

## Codexが最初に読むもの

作業開始時は、変更対象に関係なく次の順で確認してください。

1. `AGENTS.md`
2. `docs/tasks.md`
3. `docs/architecture.md`
4. 必要に応じて`docs/mobile-testing.md`
5. 長時間実行時は`docs/overnight-development.md`と`prompts/overnight.md`

ユーザーから明示的なタスク指定がある場合はその指示を最優先します。

明示的なタスク指定がない場合は、`docs/tasks.md`から依存関係を満たした最優先の未完了タスクを選んでください。

## タスク実行ルール

- 1度に実装するのは1タスクだけとする。同じタスクに必要な小さな修正はまとめてよい。
- 通常作業では1タスクを完了したら終了する。長時間実行では完了後に次の実行可能タスクへ進む。
- 先のフェーズを便利そうという理由だけで先行実装しない。
- 作業対象タスクの完了条件を先に確認してから実装する。
- 完了条件を満たした場合のみ`docs/tasks.md`のチェックボックスを`[x]`に更新する。
- 一部だけ完了した場合はチェックを付けず、必要ならタスク配下へ残課題を追記する。
- 実装中に仕様の矛盾や不足を見つけた場合、推測で大きく仕様変更せず、最小の安全な実装を選ぶ。重要な判断はPR説明に残す。
...
- DomainはFlutter WidgetとAndroid / iOSのOS APIへ依存しない。
- 日本語UIをデフォルトとする。
- Material 3を基本とする。
...
- Data: Repository実装。OS固有APIは直接使わない
- Infrastructure: ファイルアクセス、Preferences、プラットフォーム固有処理

詳細は`docs/architecture.md`を参照してください。

## 実装ルール

- ドメインロジックはWidgetに埋め込まない。
- 日付計算や状態遷移などのルールにはユニットテストを付ける。
- IDには表示名ではなくUUID等の不変IDを使う。
- 日付は内部ではタイムゾーンの曖昧さを避け、保存形式を統一する。
- Android / iOS固有コードをDomainやPresentationへ持ち込まない。
- 日本語と英字の間に半角スペースを入れない。英単語同士の半角スペースは残す。
- 破壊的なマイグレーションやデータ形式変更は、後方互換性を検討せず実施しない。

## 品質確認

コード変更後は、環境上実行可能な範囲で必ず次を実行してください。
...
Android実装を変更した場合は、可能ならAndroidビルドも確認します。

```bash
flutter build apk --debug
```

iOS実装を変更した場合は、可能ならiOSビルドも確認します。
...
コピー直後の`T-001`〜`T-003`はアプリ名・識別子など人間の入力が必要である。値が未確定ならBLOCKEDにし、機能タスクがあればそちらを進める。
...
test: add home page widget tests
fix: resolve analyzer warnings
docs: mark T-010 as blocked
```
...
PR説明または最終報告には最低限以下を含めてください。長時間実行では上記のOvernight Development Summaryも必ず付けてください。
...
- 実行したテスト / コマンドと結果
- 未完了事項や既知の制約
- `docs/tasks.md`を更新したか

## Code Review Rules

- DomainがFlutter WidgetやOS APIに依存している変更を指摘する。
- プラットフォーム固有処理がPresentation / Data / Domainへ漏れている変更を指摘する。
